...
package "api" {
  class AnalysisController {
    +analyze(file, request): ResponseEntity
    +getResult(analysisId): ResponseEntity
    +getAllResults(): ResponseEntity
    +deleteResult(analysisId): ResponseEntity
    +options(): ResponseEntity
  }
...
package "application" {
  ...
  class RateLimitService {
    +checkRateLimit(clientIp): void
    +getRemainingRequests(clientIp): int
  }
  interface LogParser {
...
package "domain" {
  class AccessLog <<record>>
  class IpInfo <<record>>
  class AnalysisResult <<@Value @Builder>>
  class ParseStatistics <<record>>
...
package "infrastructure" {
  class CsvLogParser
  class IpInfoClient
  class IpInfoCircuitBreaker
  class AnalysisRepository
}

AnalysisController --> AnalysisService
AnalysisController --> RateLimitService
AnalysisService --> LogParser
AnalysisService --> StatisticsCalculator
AnalysisService --> IpInfoClient
AnalysisService --> AnalysisResultRepository

CsvLogParser ..|> LogParser
AnalysisRepository ..|> AnalysisResultRepository

CsvLogParser ..> AccessLog : creates
IpInfoClient ..> IpInfo : creates
AnalysisService ..> AnalysisResult : creates
StatisticsCalculator ..> AccessLog : reads
@enduml
```
